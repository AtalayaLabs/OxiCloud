...
use ocm_server_axum::{
    ...
    share::{NewShareBuilder, SendShareError, into_share_result},
...
impl OpenCloudMeshService {
    ...
    pub fn get_protocols(&self) -> Arc<Vec<Box<dyn Protocol>>> {
        self.protocols.clone()
    }

    pub async fn resolve_or_create_recipient(&self, share_with: &str) -> Result<User, DomainError> {
    ...
    pub async fn send_share(
        ...
        let request = NewShareBuilder::new(
            share.id().to_string(),
            // FIXME: this is ugly
            format!(
                "{inviter_id}@{}",
        ...
        .with_shared_secret(format!("{}:{}", recipient.id(), share.token()))
        .build()?;

        let response: axum::http::Response<Vec<u8>> = reqwest::Client::new()
        ...
        let creation_response = into_share_result(response).unwrap();
...
impl From<SendShareError> for DomainError {
    fn from(value: SendShareError) -> Self {
        match value {
            ...
            SendShareError::ReceivingServerNotEnabled => {
            ...
            SendShareError::Rejected(e) => DomainError::operation_not_supported("OCM", e.message),
            SendShareError::InvalidResponse(e) => DomainError::internal_error("OCM", e),
        }
    }
}
...
trait ReqwestResponseExt {
    fn into_http_response(
        self,
    ) -> impl Future<Output = Result<axum::http::Response<Vec<u8>>, reqwest::Error>>;
}

impl ReqwestResponseExt for reqwest::Response {
    async fn into_http_response(self) -> Result<axum::http::Response<Vec<u8>>, reqwest::Error> {
