...
| `OXICLOUD_WORKER_THREADS` | CPU budget (affinity capped by the cgroup CFS quota) | Tokio worker threads. Tokio's own `TOKIO_WORKER_THREADS` is honoured as a fallback when this is unset. The default folds in `docker --cpus` / cgroup `cpu.max`, which `available_parallelism()` alone ignores — on a quota-limited container it would otherwise spawn one worker per *host* core. |
| `OXICLOUD_MAX_BLOCKING_THREADS` | `max(32, 8 × workers)` | Cap on the `spawn_blocking` pool (thumbnails, transcode, zip, PDF/text extraction, Argon2 at ~19 MB per hash). Deliberately below tokio's flat default of 512, which bounds worst-case RSS at multiple GB. |
...
| `OXICLOUD_AUTH_MIN_PASSWORD_LENGTH` | `8` | Minimum password length (UTF-8 bytes) enforced by every password-bearing endpoint: `POST /api/setup`, `POST /api/auth/register`, `POST /api/admin/users`, admin password reset, and `PATCH /api/auth/me/password`. Advertised on `GET /api/config` as `auth.min_password_length` so the SPA gates submit on the same value and users see the failure instantly instead of after a round-trip (fix for [#677](https://github.com/AtalayaLabs/OxiCloud/issues/677)). Values below 8 are accepted but emit a boot warning — OWASP interactive-auth guidance is 8+ bytes. |
...
| `OXICLOUD_ENABLE_MARKDOWN_COLLAB` | `true` | Collaborative editing of `.md` / `.txt` / code / config files (Yjs CRDT) over the message-bus WebSocket. **Requires `OXICLOUD_MESSAGEBUS_ENABLE=true`** — the CRDT data plane rides `/api/rt/ws`, so the boot refuses the contradictory combination (`enable_markdown_collab=true` with the bus off) rather than silently doing nothing. When `true`, the server wires `CollabSessionService` and the WS binary-frame branch routes Yjs sync/awareness/update frames to per-file actors backed by the `collab.doc_sessions` table; the FE mounts `CollabEditor` (CodeMirror 6 + `y-codemirror.next`) for text-shaped files, with peer cursors, per-file `can_write` gating from the subscribe ack, read-only-for-Viewers, graceful `rt.write_denied` on Update denial, and explicit flush on tab hide / unmount. Eviction surface covers `resource_deleted`, `grant_revoked`, `external_write` (WebDAV / WOPI / REST replace) and `doc_too_large` — all fire `rt.revoked` / `rt.write_denied` on the collab topic so attached editors transition to read-only within one tick. Reader seeds the CRDT from the file's current blob text on first attach; writer flushes back via a debouncer (defaults 15 s idle / 60 s max) so WebDAV / download / search see CRDT-current content within a bounded window. Per-frame AuthZ (`Read` on SYNC, `Update` on UPDATE) is enforced. Set to `false` only if you specifically don't want live Yjs routing — e.g. a read-only-clients-only deployment or one whose users write text files exclusively through external editors and expect last-write-wins semantics. Debouncer + idle-GC + doc-size-cap tuning knobs listed below. See `docs/plan/markdown-collab.md`. |
...
| `OXICLOUD_VIDEO_THUMBNAIL_CONCURRENCY` | half the CPU cores (min 1) | Maximum number of ffmpeg extractions running in parallel. |
| `OXICLOUD_VIDEO_THUMBNAIL_TIMEOUT_SECS` | `30` | Per-video ffmpeg timeout; a slower extraction is abandoned and the video gets a placeholder icon. |
| `OXICLOUD_VIDEO_THUMBNAIL_MAX_MB` | `2048` | Largest video (in MB) streamed to a temp file for frame extraction; bigger files skip thumbnailing. |
...
### Collab tuning (only relevant when `OXICLOUD_ENABLE_MARKDOWN_COLLAB=true`)

The **debouncer** flushes CRDT text back to the file's blob on two thresholds: idle (no update for `IDLE_MS`) OR max age (continuously edited for `MAX_MS` since the first dirty edit). Idle bounds "how quiet before we flush"; max bounds worst-case staleness for WebDAV/download/search on a doc that never quiets down. `TICK_MS` is the actor's wake-up cadence — smaller = faster response to thresholds, more overhead. Production defaults are Y-Sweet-shaped (15 s / 60 s / 1 s); test suites shrink them to sub-second scales.

The **idle-GC** scheduled job reaps stale `collab.doc_sessions` rows: force-flush → actor shutdown → row delete. `TTL_MINUTES` is the "how long can a session lie idle" contract; `SCAN_INTERVAL_SECS` is how tightly the contract is enforced — scanning less often than the TTL effectively extends the TTL. `BATCH_LIMIT` caps per-tick work so a large backlog can't monopolise the maintenance pool.

| Variable | Default | Description |
|---|---|---|
| `OXICLOUD_COLLAB_DEBOUNCE_IDLE_MS` | `15000` (15 s) | Debouncer idle threshold: no UPDATE for this long → flush fires. Lower = fresher blob content, more writes. |
| `OXICLOUD_COLLAB_DEBOUNCE_MAX_MS` | `60000` (60 s) | Debouncer max age: continuously edited for this long → force-fire regardless of idle. Bounds worst-case staleness for non-collab consumers. |
| `OXICLOUD_COLLAB_DEBOUNCE_TICK_MS` | `1000` (1 s) | Actor wake-up cadence for the flush-due check. Setting this larger than the idle threshold makes the idle threshold effectively meaningless. |
