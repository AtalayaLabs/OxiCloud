...
| `OXICLOUD_ENABLE_MARKDOWN_COLLAB` | `false` | Collaborative editing of `.md` files (Yjs CRDT) over the message-bus WebSocket. **Requires `OXICLOUD_MESSAGEBUS_ENABLE=true`** — the CRDT data plane rides `/api/rt/ws`, so the boot refuses the contradictory combination (`enable_markdown_collab=true` with the bus off) rather than silently doing nothing. When `true`, the server wires `CollabSessionService` and the WS binary-frame branch routes Yjs sync/awareness/update frames to per-file actors backed by the `collab.doc_sessions` table. When `false` (the default), the binary branch drops any incoming Yjs frame and a client's sync attempt times out, falling back to plain HTTP with last-write-wins semantics. **Currently opt-in for development only** — the flush-to-blob writer is stubbed (`NoopWriter`), so collaborative edits accumulate in `collab.doc_sessions` but are NOT reflected in the file's blob content until C7 (real `FileManagementService` bridge) lands. `GET /api/files/{id}` still returns the original blob, not the CRDT-current text. See `docs/plan/markdown-collab.md`. |
