...
| `OXICLOUD_ENABLE_MARKDOWN_COLLAB` | `false` | Collaborative editing of `.md` files (Yjs CRDT) over the message-bus WebSocket. **Requires `OXICLOUD_MESSAGEBUS_ENABLE=true`** — the CRDT data plane rides `/api/rt/ws`, so the boot refuses the contradictory combination (`enable_markdown_collab=true` with the bus off) rather than silently doing nothing. When `true`, the server wires `CollabSessionService` and the WS binary-frame branch routes Yjs sync/awareness/update frames to per-file actors backed by the `collab.doc_sessions` table. Backend correctness is fully wired: the reader seeds the CRDT from the file's current blob text on first attach; the writer flushes CRDT text back to the blob via a debouncer (defaults 15 s idle / 60 s max) so WebDAV / download / search see CRDT-current content within a bounded window; per-frame AuthZ (`Read` on SYNC, `Update` on UPDATE) is enforced. When `false` (the default), the binary branch drops incoming Yjs frames and clients fall back to plain HTTP with last-write-wins semantics. **Currently opt-in** — the frontend editor (CodeMirror 6 + collab extensions) hasn't landed yet, so end-users can't actually reach the feature through the UI. Debouncer + idle-GC tuning knobs listed below. See `docs/plan/markdown-collab.md`. |
